...
async function getGlobalData(ethPrice, oldEthPrice) {
  ...
  try {
    ...
    data = result.data.becoswapFactories[0]
    ...
    oneDayData = oneDayResult.data.becoswapFactories[0]

    let twoDayResult = await client.query({
      query: GLOBAL_DATA(twoDayBlock?.number),
      fetchPolicy: 'cache-first',
    })
    twoDayData = twoDayResult.data.becoswapFactories[0]

    let oneWeekResult = await client.query({
      query: GLOBAL_DATA(oneWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const oneWeekData = oneWeekResult.data.becoswapFactories[0]

    let twoWeekResult = await client.query({
      query: GLOBAL_DATA(twoWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const twoWeekData = twoWeekResult.data.becoswapFactories[0] ? twoWeekResult.data.becoswapFactories[0] : oneWeekData
...
const getChartData = async (oldestDateToFetch, offsetData) => {
  ...
  try {
    while (!allFound) {
      ...
      skip += 1000
      data = data.concat(result.data.becoswapDayDatas)
      if (result.data.becoswapDayDatas.length < 1000) {
        allFound = true
      }
    }
